...
def simple_c_solver(p, p_prime, u_e, v_n, a_p, flow_regions, x_nums, y_nums, delta_x, delta_y, density, inlet_velocity, inner_epochs, uv_alpha, p_alpha):
    ...
    for epoch in range(inner_epochs):
        ...
        p_prime[2: x_nums + 2, 2: y_nums + 2] \
            = ((a_pe * p_prime[3: x_nums + 3, 2: y_nums + 2] + a_pw * p_prime[1: x_nums + 1, 2: y_nums + 2]
                ...
                + (v_n[2: x_nums + 2, 1: y_nums + 1] - v_n[2: x_nums + 2, 2: y_nums + 2]) * delta_x)
               / a_pp)

        p_prime[2: x_nums + 2, 2: y_nums + 2] = torch.where(flow_regions[2: x_nums + 2, 2: y_nums + 2].eq(0), 0.0, p_prime[2: x_nums + 2, 2: y_nums + 2])
