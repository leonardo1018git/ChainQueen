...
def simple_solver(p, p_prime, u_e, v_n, a_p, flow_regions, x_nums, y_nums, delta, density, inlet_velocity, inner_epochs, uv_alpha, p_alpha):
    ...
    a_pe = torch.where(flow_regions[3: x_nums + 3, 2: y_nums + 2].eq(0)
                       ...
                       uv_alpha * (1.0 / a_p[3: x_nums + 3, 2: y_nums + 2] + 1.0 / a_p[2: x_nums + 2, 2: y_nums + 2])
                       * delta ** 2 / (2 * density * inlet_velocity ** 2))

    a_pw = torch.where(flow_regions[1: x_nums + 1, 2: y_nums + 2].eq(0)
                       | flow_regions[2: x_nums + 2, 2: y_nums + 2].eq(0), 1.0e-30,
                       uv_alpha * (1.0 / a_p[1: x_nums + 1, 2: y_nums + 2] + 1.0 / a_p[2: x_nums + 2, 2: y_nums + 2])
                       * delta ** 2 / (2 * density * inlet_velocity ** 2))

    a_pn = torch.where(flow_regions[2: x_nums + 2, 3: y_nums + 3].eq(0)
                       | flow_regions[2: x_nums + 2, 2: y_nums + 2].eq(0), 1.0e-30,
                       uv_alpha * (1.0 / a_p[2: x_nums + 2, 3: y_nums + 3] + 1.0 / a_p[2: x_nums + 2, 2: y_nums + 2])
                       * delta ** 2 / (2 * density * inlet_velocity ** 2))

    a_ps = torch.where(flow_regions[2: x_nums + 2, 1: y_nums + 1].eq(0)
                       | flow_regions[2: x_nums + 2, 2: y_nums + 2].eq(0), 1.0e-30,
                       uv_alpha * (1.0 / a_p[2: x_nums + 2, 1: y_nums + 1] + 1.0 / a_p[2: x_nums + 2, 2: y_nums + 2])
                       * delta ** 2 / (2 * density * inlet_velocity ** 2))
    ...
    for epoch in range(inner_epochs):
        ...
        p_prime[2: x_nums + 2, 2: y_nums + 2] \
            = ((a_pe * p_prime[3: x_nums + 3, 2: y_nums + 2] + a_pw * p_prime[1: x_nums + 1, 2: y_nums + 2]
                ...
                + (u_e[1: x_nums + 1, 2: y_nums + 2] - u_e[2: x_nums + 2, 2: y_nums + 2]) * delta
                + (v_n[2: x_nums + 2, 1: y_nums + 1] - v_n[2: x_nums + 2, 2: y_nums + 2]) * delta) / a_pp)
        ...
        # print("epoch = " + str(epoch) + ", pError = " + str(p_error))
        if p_error < 1.0e-6:
            break

        p_error_error = abs(p_error_old - p_error) * 100 / max(p_error, 1.0e-30)
        if p_error_error < 1.0e-6 and p_error is not 100.0 and math.isnan(p_error) is False:
            break
        p_error_old = p_error
